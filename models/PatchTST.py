...
from models.modules.Transformer_EncDec import Encoder, EncoderLayer
from models.modules.SelfAttention_Family import FullAttention, AttentionLayer
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'spg':
            ...
            self.projection = nn.Linear(
                self.head_nf * configs.enc_in, 230)
        elif self.task_name == 'crysystem':
            self.flatten = nn.Flatten(start_dim=-2)
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(
                self.head_nf * configs.enc_in, 7)
